...
# --- FUNGSI WARNA STATUS GIZI (IMT) ---
def warna_imt(val):
    try:
        val = float(val)
        if val < 18.5: color = '#F3C623' # Kuning (Kurus)
        elif 18.5 <= val < 25.0: color = '#9BDBA1' # Hijau (Normal)
        elif 25.0 <= val < 30.0: color = '#FFA447' # Orange (Overweight)
        else: color = '#FF6969' # Merah (Obesitas)
...
if not st.session_state['login_berhasil']:
    ...
    with tab1:
        with st.form("form_identitas", clear_on_submit=True): # <--- FITUR AUTO RESET
    ...
    with tab_ncp:
        ...
        if not df_identitas.empty:
            ...
            if list_p:
                nama_pilih = st.selectbox("Pilih Pasien Dari Database:", options=list_p)
                ...
                st.warning("Anda belum memiliki data pasien untuk diinput klinisnya.")

    # --- FILTER TAHUN ---
    tahun_skrg = datetime.now().year
    list_tahun = list(range(1900, tahun_skrg + 11))

    # --- TAB 3: REKAP IDENTITAS (Dengan Warna & Filter Ringkas) ---
    with tab2:
        if not df_identitas.empty:
            # Hak Akses Data
            if st.session_state['username'] == "ardilla":
                df_view = df_identitas.copy()
            else:
                df_view = df_identitas[df_identitas['input_by'] == st.session_state['username']].copy()

            df_view['tgl_mrs'] = pd.to_datetime(df_view['tgl_mrs'], errors='coerce')
            f1, f2, f3 = st.columns(3)
            with f1: bul_i = st.selectbox("Bulan", range(1, 13), index=datetime.now().month-1, format_func=lambda x: datetime(tahun_skrg, x, 1).strftime('%B'))
            with f2: tah_i = st.selectbox("Tahun", list_tahun, index=list_tahun.index(tahun_skrg))
            with f3: rng_i = st.multiselect("Ruangan (Kosongkan = Semua)", options=list_ruang)

            # LOGIKA FILTER RINGKAS
            sel_rooms = rng_i if rng_i else list_ruang

            mask_i = (df_view['tgl_mrs'].dt.month == bul_i) & (df_view['tgl_mrs'].dt.year == tah_i) & (df_view['ruang'].isin(sel_rooms))
            df_f_i = df_view[mask_i]
            
            st.markdown(f"<div class='metric-box'>📊 Terfilter: <b>{len(df_f_i)}</b> | Total Milik Anda: <b>{len(df_view)}</b></div>", unsafe_allow_html=True)
            
            # --- TAMPILAN DENGAN WARNA IMT ---
            st.dataframe(df_f_i.style.applymap(warna_imt, subset=['imt']), use_container_width=True, hide_index=True)
            
            out_i = BytesIO()
            with pd.ExcelWriter(out_i, engine='openpyxl') as writer:
                df_f_i.to_excel(writer, index=False, sheet_name='Identitas')
            st.download_button("📥 DOWNLOAD EXCEL", data=out_i.getvalue(), file_name=f"Rekap_Gizi.xlsx")

    # --- TAB 4: REKAP KLINIS ---
    with tab_rekap_ncp:
        if not df_ncp.empty:
            if st.session_state['username'] == "ardilla":
                df_view_k = df_ncp.copy()
            else:
                df_view_k = df_ncp[df_ncp.get('input_by','') == st.session_state['username']].copy()

            df_view_k['tgl_input'] = pd.to_datetime(df_view_k['tgl_input'], errors='coerce')
            k1, k2, k3 = st.columns(3)
            with k1: bul_k = st.selectbox("Bulan (Klinis)", range(1, 13), index=datetime.now().month-1, format_func=lambda x: datetime(tahun_skrg, x, 1).strftime('%B'))
            with k2: tah_k = st.selectbox("Tahun (Klinis)", list_tahun, index=list_tahun.index(tahun_skrg))
            with k3: rng_k = st.multiselect("Ruangan (Klinis - Kosongkan=Semua)", options=list_ruang)

            sel_rooms_k = rng_k if rng_k else list_ruang

            mask_k = (df_view_k['tgl_input'].dt.month == bul_k) & (df_view_k['tgl_input'].dt.year == tah_k) & (df_view_k['ruang'].isin(sel_rooms_k))
            df_f_k = df_view_k[mask_k]
            
            st.markdown(f"<div class='metric-box'>📜 Catatan Terfilter: <b>{len(df_f_k)}</b></div>", unsafe_allow_html=True)
