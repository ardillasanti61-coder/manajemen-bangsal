...
st.markdown("""
    ...
    * { cursor: url("https://img.icons8.com/emoji/32/tangerine-emoji.png"), auto !important; }
    ...
    .btn-hapus > button {
        background: #FF6969 !important; color: white !important; height: 2.5em !important; width: 100% !important;
    }
    .metric-box {
...
if not st.session_state['login_berhasil']:
        ...
        with st.form("form_identitas", clear_on_submit=True):
            ...
            if st.form_submit_button("SIMPAN DATA IDENTITAS"):
                if rm and nama:
                    ...
                    df_identitas = pd.concat([df_identitas, new_id], ignore_index=True)
    ...
    # --- TAB 2: DATA KLINIS ---
    with tab_ncp:
        # Filter pasien agar user hanya melihat pasien yang mereka input (kecuali ardilla)
        if user_aktif == "ardilla":
            list_p = df_identitas['nama_pasien'].tolist()
        else:
            list_p = df_identitas[df_identitas['input_by'].str.lower() == user_aktif]['nama_pasien'].tolist()
        
        if list_p:
            nama_pilih = st.selectbox("Pilih Pasien:", options=list_p)
            row_p = df_identitas[df_identitas['nama_pasien'] == nama_pilih].iloc[0]
            with st.form("form_klinis"):
                ...
                if st.form_submit_button("SIMPAN DATA KLINIS"):
                    ...
                    df_ncp = pd.concat([df_ncp, new_ncp], ignore_index=True)
                    conn.update(worksheet="NCP", data=df_ncp)
                    st.success("Data Klinis Tersimpan!"); st.rerun()
        else:
            st.warning("Belum ada data pasien yang diinput oleh akun ini.")

    # --- TAB 3: REKAP & HAPUS IDENTITAS ---
    with tab2:
        st.subheader("📊 Rekap & Filter Identitas")
        f1, f2, f3 = st.columns(3)
        with f1: bul_i = st.selectbox("Bulan MRS", range(1, 13), index=datetime.now().month-1, key="bi")
        with f2: tah_i = st.selectbox("Tahun MRS", range(2024, 2031), index=0, key="ti")
        with f3: rng_i = st.multiselect("Filter Ruang", list_ruang)

        df_identitas['tgl_mrs_dt'] = pd.to_datetime(df_identitas['tgl_mrs'], errors='coerce')
        mask_i = (df_identitas['tgl_mrs_dt'].dt.month == bul_i) & (df_identitas['tgl_mrs_dt'].dt.year == tah_i)
        ...
        df_filtered_i = df_identitas[mask_i].copy()

        if not df_filtered_i.empty:
            st.markdown(f"<div class='metric-box'>Terdata: <b>{len(df_filtered_i)}</b> Pasien</div>", unsafe_allow_html=True)
            
            # Tombol Hapus per Baris
            for idx, row in df_filtered_i.iterrows():
                with st.expander(f"👤 {row['nama_pasien']} ({row['ruang']})"):
                    c_txt, c_btn = st.columns([4, 1])
                    c_txt.write(f"RM: {row['no_rm']} | Kamar: {row['no_kamar']} | IMT: {row['imt']}")
                    if c_btn.button("🗑️ Hapus", key=f"del_id_{idx}"):
                        df_identitas = df_identitas.drop(idx)
                        conn.update(worksheet="Sheet1", data=df_identitas.drop(columns=['tgl_mrs_dt']))
                        st.warning("Data Berhasil Dihapus!"); st.rerun()
            
            st.dataframe(df_filtered_i.drop(columns=['tgl_mrs_dt']).style.applymap(warna_imt, subset=['imt']), use_container_width=True)
        else:
            st.info("Tidak ada data identitas yang cocok dengan filter.")

    # --- TAB 4: REKAP KLINIS ---
    with tab_rekap_ncp:
        st.subheader("📜 Rekap Catatan Klinis")
        k1, k2 = st.columns(2)
        with k1: bul_k = st.selectbox("Bulan Input", range(1, 13), index=datetime.now().month-1, key="bk")
        with k2: tah_k = st.selectbox("Tahun Input", range(2024, 2031), index=0, key="tk")

        df_ncp['tgl_input_dt'] = pd.to_datetime(df_ncp['tgl_input'], errors='coerce')
        mask_k = (df_ncp['tgl_input_dt'].dt.month == bul_k) & (df_ncp['tgl_input_dt'].dt.year == tah_k)
        if user_aktif != "ardilla": mask_k = mask_k & (df_ncp['input_by'].str.lower() == user_aktif)
        
        df_filtered_k = df_ncp[mask_k].copy()

        if not df_filtered_k.empty:
            st.markdown(f"**Ditemukan {len(df_filtered_k)} catatan klinis**")
            st.dataframe(df_filtered_k.drop(columns=['tgl_input_dt']), use_container_width=True, hide_index=True)
            
            # Download Button
            out = BytesIO()
            with pd.ExcelWriter(out, engine='openpyxl') as writer:
                df_filtered_k.drop(columns=['tgl_input_dt']).to_excel(writer, index=False)
            st.download_button("📥 Download Excel Klinis", data=out.getvalue(), file_name=f"Klinis_{bul_k}_{tah_k}.xlsx")
        else:
            st.info("Tidak ada data klinis untuk bulan/tahun ini.")
